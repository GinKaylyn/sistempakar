<x-app-layout>
	<x-slot name="title">Hasil Sistem Pakar</x-slot>

	@if(session()->has('success'))
	<x-alert type="success" message="{{ session()->get('success') }}" />
	@endif
	<x-card title="Berikut Hasil Perhitungan Sistem Pakar">
		<p class="mb-4">
		...
		<div class="card card-body p-0 mt-5 border" style="box-shadow: none !important;">
			<div class="card-header bg-primary text-white p-2">
				<h6 class="font-weight-bold">Tabel perhitungan Sistem Pakar: {{ $diagnosa['nama_penyakit'] }} ({{ $diagnosa['kode_penyakit'] }})</h6>
			</div>
		...
		<div class="mt-5">
			<div class="alert alert-success">
				...
				<p>Berdasarkan dari kriteria yang anda pilih dan juga berdasarkan Rule/Basis aturan yang telah kami teliti maka perhitungan Algoritma 
					Certainty Factor mengambil nilai CF yang paling pinggi yakni <b>{{ number_format(unserialize($riwayat->cf_max)[0], 3) }} 
					({{ number_format(unserialize($riwayat->cf_max)[0], 3) * 100 }}%)</b> yaitu <b>{{ unserialize($riwayat->cf_max)[1] }}</b></p>
			</div>
